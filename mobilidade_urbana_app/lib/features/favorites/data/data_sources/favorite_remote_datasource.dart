...
class FavoriteRemoteDataSourceImpl implements FavoriteRemoteDatasource {
  ...
  Future<FavoriteModel> addFavorite(Map<String, dynamic> data) async {
    final response = await _dio.post('/profiles/favorites/routes', data: data);
    return FavoriteModel.fromJson(response.data);
  }

  @override
  Future<List<FavoriteModel>> getFavorites() async {
    final response = await _dio.get('/profiles/favorites/routes');
    return (response.data as List).map((json) => FavoriteModel.fromJson(json)).toList();
  }

  @override
  Future<void> deleteFavorite(String favoriteId) async {
    await _dio.delete('/profiles/favorites/routes/$favoriteId');
  }

  @override
  Future<FavoriteModel> updateFavorite(String favoriteId, Map<String, dynamic> data) async {
    final response = await _dio.put('/profiles/favorites/routes/$favoriteId', data: data);
    return FavoriteModel.fromJson(response.data);
  }
}